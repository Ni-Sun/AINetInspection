                    ...
                    <el-row :gutter="20" class="billboards-left-top">
                        <el-col :span="8">
                            <div class="card stats-digit">
                                <div class="card-body">
                                    <div class="stats-label">机器狗布控</div>
                                    <div class="stats-value">{{ statistics.deviceCount }}</div>
                                </div>
                                <div class="card-icon text-primary">
                                    <!-- 引入机器狗png 限制缩放28*28 -->
                                    <img :src="dogImage" alt="机器狗" style="width: 28px; height: 28px;">
                                </div>
                            </div>
                            ...
                            <div class="layout card">
                                <div class="layout-top flex-between">
                                    <div class="title"><i class="el-icon-video-camera"
                                            style="vertical-align: text-bottom;"></i>机器狗实时监控画面</div>
                                    <div class="action">
                                        ...
                                        <div class="create-button" v-if="locationList.length == 0">
                                            <el-button type="text" @click="createDevice">添加机器狗</el-button>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="layout card">
                                ...
                                <div class="layout-content" v-loading="alarmLoading">
                                    <div class="simple-list">
                                        <div class="simple-wrap" v-if="staticAlarmList.length > 0">
                                            <vue-seamless-scroll ref="scrollBar" :data="staticAlarmList" :loop="true"
                                                :class-option="classOption">
                                                <ul class="list-unstyled"
                                                    :class="{ 'activity-wid': staticAlarmList.length > 1 }">
                                                    <li class="activity-list" v-for="(item, index) in staticAlarmList"
                                                        :key="index">
...
export default {
    ...
    data() {
        return {
            dogImage: require("@/assets/images/logo.png"),
            loading: false,
            countDuration: 2000,
            statistics: {
                deviceCount: 0,
            ...
            staticAlarmList: [
                {
                    alertId: "ALM-1001",
                    alertLevel: "高",
                    alertTime: "2025-12-19 10:12:34",
                    alertType: "入侵检测",
                    cameraPosition: "门口-北侧",
                    capturedImage: require("@/assets/images/logo.png"),
                    capturedVideo: "",
                    monitoringTask: "巡检任务-A"
                },
                {
                    alertId: "ALM-1002",
                    alertLevel: "中",
                    alertTime: "2025-12-19 09:22:11",
                    alertType: "跌倒检测",
                    cameraPosition: "广场-东侧",
                    capturedImage: require("@/assets/images/logo.png"),
                    capturedVideo: "",
                    monitoringTask: "巡检任务-B"
                },
                {
                    alertId: "ALM-1002",
                    alertLevel: "中",
                ...
                },
                {
                    alertId: "ALM-1003",
                    alertLevel: "低",
                    alertTime: "2025-12-18 18:05:00",
                    alertType: "异常徘徊",
                    cameraPosition: "货区-2号",
                    capturedImage: require("@/assets/images/logo.png"),
                    capturedVideo: "",
                    monitoringTask: "巡检任务-C"
                }
            ],
            isfirst: true,
            classOption: {
    ...
    methods: {
        ...
        viewDetail(row) {
            ...
            try {
                const found = this.staticAlarmList.find(item => item.alertId == row.alertId);
                if (found) {
                    // 复制一份避免修改原数组
                    this.alarmInfo = Object.assign({}, found);
                } else {
                    // 若未找到，基于传入行构建一个临时详情
                    this.alarmInfo = {
                        alertId: row.alertId || "UNKNOWN",
                        alertLevel: row.alertLevel || "中",
                        alertTime: row.alertTime || new Date().toLocaleString(),
                        alertType: row.alertType || "未知类型",
                        cameraPosition: row.cameraPosition || "未指定",
                        capturedImage: this.dogImage,
                        capturedVideo: "",
                        monitoringTask: row.monitoringTask || "默认任务"
                    };
                }
